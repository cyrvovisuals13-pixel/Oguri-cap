...
import pkg from 'whatsapp-web.js/src/structures/index.js';
const { Message, MessageMedia, Contact, Location } = pkg;
...
const serialize = async (oguri, m) => {
    ...
    if (oguri.info) m.botNumber = oguri.info.me._serialized || oguri.info.wid._serialized
    ...
    if (m.isGroup) {
        m.metadata = await (await oguri.groupMetadata(m.from))
    ...
    if (m.isMedia) m.downloadMedia = (filePath) => {
        if (filePath) return oguri.downloadAndSaveMediaMessage(m, filePath)
        else return oguri.downloadMediaMessage(m)
    }
    m.resend = () => oguri.forwardMessage(m.from, m._serialized)
    m.reply = (content, options = {}) => {
        if (!!config.msg[content]) content = config.msg[content]
        oguri.sendMessage(options.from ? options.from : m.from, content, { quoted: m, ...options })
    }
    ...
    if (m.hasQuotedMsg) {
        ...
        m.quoted = await (await serialize(oguri, data))
